...
// Изначально окно удаления клиента находилось внутри скрываемой страницы товаров.
// Переносим его в body, чтобы оно открывалось из вкладки "Клиенты".
if (deleteClientModal) {
    document.body.appendChild(deleteClientModal);
}
...
function renderProducts() {
    ...
    const filteredProducts =
        allProducts.filter(product => {
            ...
            const matchesSearch =
                !searchText || productSearchText.includes(searchText);
...
async function loadClients() {
    ...
    table.innerHTML = `<tr><td colspan="8">Загрузка данных...</td></tr>`;

    try {
        const response = await fetch("/api/clients");
        if (!response.ok) throw new Error("Ошибка получения клиентов");

        allClients = await response.json();
        renderClients();
    } catch (error) {
        console.error(error);
        table.innerHTML = `<tr><td colspan="8">Не удалось загрузить клиентов.</td></tr>`;
    }
}
...
function renderClients() {
    ...
    const clients = allClients.filter(client => {
        const text = [
            client.id, client.name, client.type, client.inn,
            client.phone, client.email, client.deliveryAddress
        ].join(" ").toLowerCase();
        return !search || text.includes(search);
    });

    if (!clients.length) {
        table.innerHTML = `<tr><td colspan="8">Клиенты не найдены.</td></tr>`;
        return;
    }
        ...
        <tr>
            ...
            <td class="table-actions">
                <button type="button" class="action-button edit-button"
                        onclick="editClient(${client.id})" title="Редактировать">✏️</button>
...
async function loadEmployees() {
    ...
    table.innerHTML = `<tr><td colspan="7">Загрузка данных...</td></tr>`;
    ...
    try {
        ...
        table.innerHTML = `<tr><td colspan="7">Не удалось загрузить сотрудников.</td></tr>`;
    }
}
...
function renderEmployees() {
    ...
    const employees = allEmployees.filter(employee => {
        ...
        const text = [
            employee.id, employee.name, employee.positionName, hireDate,
            employee.salary, employee.departmentName
        ].join(" ").toLowerCase();
        return !search || text.includes(search);
    });

    if (!employees.length) {
        table.innerHTML = `<tr><td colspan="7">Сотрудники не найдены.</td></tr>`;
        return;
    }
    ...
    table.innerHTML = employees.map(employee => {
        const hireDate = employee.hireDate
            ? new Date(employee.hireDate).toLocaleDateString("ru-RU") : "—";
        const salary = Number(employee.salary || 0).toLocaleString("ru-RU", {
        ...
        return `
            <tr>
                ...
                <td class="table-actions">
                    <button type="button" class="action-button edit-button"
                            onclick="editEmployee(${employee.id})" title="Редактировать">✏️</button>
                    <button type="button" class="action-button delete-button"
                            onclick="deleteEmployee(${employee.id})" title="Удалить">🗑️</button>
                </td>
            </tr>`;
    }).join("");
}
...
async function loadStockFormData() {
    const [warehousesResponse, productsResponse] = await Promise.all([
        fetch("/api/stock/warehouses"),
        fetch("/api/products")
    ]);

    if (!warehousesResponse.ok) throw new Error("Не удалось загрузить склады.");
    if (!productsResponse.ok) throw new Error("Не удалось загрузить товары.");

    const warehouses = await warehousesResponse.json();
    const products = await productsResponse.json();

    document.getElementById("stockWarehouse").innerHTML = warehouses
        .map(item => `<option value="${item.id}">${escapeHtml(item.name)}</option>`)
        .join("");

    document.getElementById("stockProduct").innerHTML = products
        .map(item => `<option value="${item.id}">${escapeHtml(item.name)}</option>`)
        .join("");
}

function closeStockModalWindow() {
    stockModal?.classList.add("hidden");
    stockForm?.reset();
    const id = document.getElementById("stockId");
    if (id) id.value = "";
}

addStockButton?.addEventListener("click", async () => {
    try {
        await loadStockFormData();
        stockForm.reset();
        document.getElementById("stockId").value = "";
        document.getElementById("stockQuantity").value = "0";
        document.getElementById("stockMinimumQuantity").value = "0";
        stockModalTitle.textContent = "Новая складская позиция";
        stockModal.classList.remove("hidden");
    } catch (error) {
        showToast(error.message || "Не удалось открыть форму.", "error");
    }
});

closeStockModal?.addEventListener("click", closeStockModalWindow);
cancelStock?.addEventListener("click", closeStockModalWindow);
stockModal?.addEventListener("click", event => {
    if (event.target === stockModal) closeStockModalWindow();
});

async function editStock(id) {
    try {
        const item = allStock.find(row => Number(row.id) === Number(id));
        if (!item) throw new Error("Складская позиция не найдена.");

        await loadStockFormData();
        document.getElementById("stockId").value = item.id;
        document.getElementById("stockWarehouse").value = item.warehouseId;
        document.getElementById("stockProduct").value = item.productId;
        document.getElementById("stockQuantity").value = item.quantity;
        document.getElementById("stockMinimumQuantity").value = item.minimumQuantity;
        stockModalTitle.textContent = "Редактирование складской позиции";
        stockModal.classList.remove("hidden");
    } catch (error) {
        showToast(error.message || "Не удалось открыть складскую позицию.", "error");
    }
}
...
stockForm?.addEventListener("submit", async event => {
    ...
    const id = Number(document.getElementById("stockId").value || 0);
    const data = {
        warehouseId: Number(document.getElementById("stockWarehouse").value),
        productId: Number(document.getElementById("stockProduct").value),
        quantity: Number(document.getElementById("stockQuantity").value),
        minimumQuantity: Number(document.getElementById("stockMinimumQuantity").value)
    };

    if (!data.warehouseId || !data.productId || data.quantity < 0 || data.minimumQuantity < 0) {
        showToast("Проверьте заполнение полей складской позиции.", "error");
        return;
    }

    try {
        const response = await fetch(id ? `/api/stock/${id}` : "/api/stock", {
            method: id ? "PUT" : "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(data)
        });
        const result = await response.json().catch(() => null);
        if (!response.ok) throw new Error(result?.message || result?.error || "Ошибка сохранения.");

        closeStockModalWindow();
        showToast(id ? "Складская позиция изменена." : "Складская позиция добавлена.", "success");
        await loadStock();
    } catch (error) {
        showToast(error.message || "Не удалось сохранить складскую позицию.", "error");
    }
});

async function deleteStock(id) {
    if (!confirm("Удалить выбранную складскую позицию?")) return;

    try {
        const response = await fetch(`/api/stock/${id}`, { method: "DELETE" });
        const result = await response.json().catch(() => null);
        if (!response.ok) throw new Error(result?.message || result?.error || "Ошибка удаления.");

        showToast("Складская позиция удалена.", "success");
        await loadStock();
    } catch (error) {
        showToast(error.message || "Не удалось удалить складскую позицию.", "error");
    }
}
...
async function loadOrders() {
    ...
    try {
        ...
        console.error(error);
        table.innerHTML = `<tr><td colspan="7">Не удалось загрузить заказы.</td></tr>`;
    }
}
...
function renderOrders() {
    ...
    const orders = allOrders.filter(order => {
        ...
        const text = [
            order.id, date, order.clientName, order.employeeName,
    ...
    if (!orders.length) {
        table.innerHTML = `<tr><td colspan="7">Заказы не найдены.</td></tr>`;
        return;
    }
    ...
    table.innerHTML = orders.map(order => {
        ...
        return `
            <tr>
                ...
                <td>${Number(order.total).toLocaleString("ru-RU", {minimumFractionDigits: 2})} ₽</td>
                <td class="table-actions">
                    <button class="details-button" type="button"
                            onclick="showOrderDetails(${order.id})">Подробнее</button>
                    <button class="action-button edit-button" type="button"
                            onclick="editOrder(${order.id})" title="Редактировать">✏️</button>
                    <button class="action-button delete-button" type="button"
                            onclick="openDeleteOrderModal(${order.id})" title="Удалить">🗑️</button>
                </td>
            </tr>`;
    }).join("");
}
...
async function loadStock() {
    ...
    table.innerHTML = `<tr><td colspan="6">Загрузка данных...</td></tr>`;

    try {
        const response = await fetch("/api/stock");
        if (!response.ok) throw new Error("Ошибка получения складских остатков");
        ...
        console.error(error);
        table.innerHTML = `<tr><td colspan="6">Не удалось загрузить складские остатки.</td></tr>`;
    }
}
...
function renderStock() {
    ...
    const stock = allStock.filter(item => {
        const text = [
            item.id, item.warehouse, item.product, item.alcoholType,
    ...
    if (!stock.length) {
        table.innerHTML = `<tr><td colspan="6">Складские позиции не найдены.</td></tr>`;
        return;
    }
    ...
    table.innerHTML = stock.map(item => `
        <tr>
            ...
            <td>${escapeHtml(item.alcoholType)}</td>
            <td>${item.quantity}</td>
            <td>${item.minimumQuantity}</td>
            <td class="table-actions">
                <button type="button" class="action-button edit-button"
                        onclick="editStock(${item.id})" title="Редактировать">✏️</button>
                <button type="button" class="action-button delete-button"
                        onclick="deleteStock(${item.id})" title="Удалить">🗑️</button>
            </td>
        </tr>`).join("");
}
...
if (addOrderButton) {
    ...
    addOrderButton.addEventListener(
        ...
        async () => {
            ...
            if (orderDateInput) orderDateInput.value = currentDateTimeLocal();
            if (orderStatusSelect) orderStatusSelect.value = "Новый";
...
async function editOrder(orderId) {
    try {
        ...
        if (!response.ok || !order) {
            throw new Error(order?.message || order?.error || "Не удалось загрузить заказ.");
        }

        document.getElementById("orderClientSelect").value = String(order.clientId);
        document.getElementById("orderEmployeeSelect").value = String(order.employeeId);
        document.getElementById("orderStatus").value = order.status || "Новый";
        document.getElementById("orderDate").value = toDateTimeLocal(order.orderDate);

        const productSelect = document.getElementById("orderProductSelect");
        orderItems = (order.items || []).map(item => {
            const option = Array.from(productSelect.options).find(
                option => Number(option.value) === Number(item.productId));

            return {
                productId: Number(item.productId),
                name: option?.dataset.name || `Товар №${item.productId}`,
        ...
        const title = orderModalCreate.querySelector(".modal-header h2");
        if (title) title.textContent = `Редактирование заказа №${orderId}`;

        const submitButton = orderForm.querySelector('button[type="submit"]');
        if (submitButton) submitButton.textContent = "Сохранить изменения";

        orderModalCreate.classList.remove("hidden");
    } catch (error) {
        console.error("Ошибка открытия заказа:", error);
        editingOrderId = null;
        showToast(error.message || "Не удалось открыть заказ для редактирования.", "error");
    }
}
...
if (orderForm) {
    ...
    orderForm.addEventListener(
        ...
        async (event) => {
            ...
            if (!orderDate) {
                showToast("Укажите дату и время заказа.", "error");
                return;
            }
