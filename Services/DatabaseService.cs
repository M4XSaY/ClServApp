...
namespace ClServApp.Server.Services
{
    ...
    """;
        ...
        public async Task UpdateClientAsync(int id, ClientCreate client)
        {
            if (id <= 0)
            ...
            if (string.IsNullOrWhiteSpace(client.Name))
                throw new ArgumentException("Наименование клиента не может быть пустым.");

            if (string.IsNullOrWhiteSpace(client.Type))
                throw new ArgumentException("Необходимо указать тип клиента.");
        ...
        """;
            ...
            command.Parameters.AddWithValue("inn",
                string.IsNullOrWhiteSpace(client.Inn) ? DBNull.Value : client.Inn.Trim());
            command.Parameters.AddWithValue("phone", client.Phone.Trim());
            command.Parameters.AddWithValue("email",
                string.IsNullOrWhiteSpace(client.Email) ? DBNull.Value : client.Email.Trim());
            command.Parameters.AddWithValue("address", client.Address.Trim());

            var affectedRows = await command.ExecuteNonQueryAsync();
            if (affectedRows == 0)
                throw new KeyNotFoundException("Клиент с указанным ID не найден.");
        }
        ...
        SELECT
            ...
            т.наименование,
            т.тип_алкоголя::text,
            со.текущее_количество,
            со.мин_порог_запаса
        FROM "Складской остаток" со
        INNER JOIN "Склад" с ON с.id = со.id_склада
        INNER JOIN "Товар" т ON т.id = со.id_товара
        ORDER BY с.название, т.наименование;
        """;
            ...
            while (await reader.ReadAsync())
            {
                stock.Add(new StockItem
                {
                    ...
                    AlcoholType = reader.GetString(5),
                    Quantity = reader.GetInt32(6),
                    MinimumQuantity = reader.GetInt32(7)
                });
            }
        ...
        public async Task<List<(int Id, string Name)>> GetWarehousesAsync()
        {
            var warehouses = new List<(int Id, string Name)>();
        ...
        """;
            ...
            while (await reader.ReadAsync())
                warehouses.Add((reader.GetInt32(0), reader.GetString(1)));

            return warehouses;
        }

        public async Task<int> AddStockAsync(StockCreate item)
        {
            ...
            const string sql = """
        INSERT INTO "Складской остаток"
            (id_склада, id_товара, текущее_количество, мин_порог_запаса)
        VALUES
        ...
        """;
            ...
            await using var command = new NpgsqlCommand(sql, connection);
            command.Parameters.AddWithValue("warehouse_id", item.WarehouseId);
            command.Parameters.AddWithValue("product_id", item.ProductId);
            command.Parameters.AddWithValue("quantity", item.Quantity);
            command.Parameters.AddWithValue("minimum_quantity", item.MinimumQuantity);
            ...
            {
                throw new InvalidOperationException(
                    "Для выбранного склада и товара складская позиция уже существует.");
            }
        }

        public async Task UpdateStockAsync(int id, StockCreate item)
        {
            if (id <= 0)
                throw new ArgumentException("Некорректный ID складской позиции.");
            ...
            const string sql = """
        UPDATE "Складской остаток"
        SET
            id_склада = @warehouse_id,
            id_товара = @product_id,
            текущее_количество = @quantity,
            мин_порог_запаса = @minimum_quantity
        WHERE id = @id;
        """;
            ...
            try
            {
                var affectedRows = await command.ExecuteNonQueryAsync();
                if (affectedRows == 0)
                    throw new KeyNotFoundException("Складская позиция не найдена.");
            }
            catch (PostgresException ex) when (ex.SqlState == PostgresErrorCodes.UniqueViolation)
            {
                throw new InvalidOperationException(
        ...
        private static void ValidateStockItem(StockCreate item)
        {
            if (item == null)
            ...
            if (item.Quantity < 0)
                throw new ArgumentException("Текущее количество не может быть отрицательным.");
            if (item.MinimumQuantity < 0)
                throw new ArgumentException("Минимальный запас не может быть отрицательным.");
        }
        ...
        public async Task AddOrderAsync(OrderCreate order)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(order.Status) ||
                    !new[] { "Новый", "Сборка", "Отгружен", "Отменен" }.Contains(order.Status))
                    throw new Exception("Указан некорректный статус заказа.");
    ...
    OrderEdit order)
        ...
        """;
                ...
                if (order.OrderDate == default)
                    throw new Exception("Укажите дату заказа.");
        ...
        SET
            ...
            статус = CAST(@status AS order_status)
        WHERE id = @order_id;
        """;
